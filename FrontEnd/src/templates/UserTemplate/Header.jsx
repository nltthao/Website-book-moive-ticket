...
export default () => {
    ...
    useEffect(() => {
        // Hàm này chạy sau khi component đã được mount vào DOM
        const headerElement = document.getElementById('header');
        if (headerElement) {
            headerElement.style.backgroundColor = 'blue'; // Ví dụ về cách sử dụng thuộc tính style
        }
    }, []);
    const showDrawer = () => {
        setOpen(true);
                ...
                <div>
                    {localStorage.getItem('token') ? <>
                        <NavLink to='/inforUser' className="flex flex-col items-center justify-center">
                            ...
                            <div>
                                <h5 className='m-0 pl-2 text-center'>Hi! {username}</h5>
                            </div>
                        </NavLink>
